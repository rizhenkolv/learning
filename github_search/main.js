class View{
    ...
    createUser(userData){
        ...
        userElement.innerHTML = `<img class="user-prev-foto" src="${userData.avatar_url}" alt="${userData.login}">
...
class Search {
    ...
    async searchUsers(){
        return await fetch(`https://api.github.com/search/users?q=${this.view.searchInput.value}`).then(
            (res)=>{
                if(res.ok){
                    res.json().then(res=>{
